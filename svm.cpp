...
class SVC_Q : public Kernel {
 public:
  SVC_Q(const svm_problem &prob, const svm_parameter &param, const schar *y_) : Kernel(prob.l, prob.x, param) {
    clone(y, y_, prob.l);
...
// Binning method from the oneclass_prob paper by Que and Lin to predict the probability as a normal instance (i.e., not
// an outlier)
...
static int svm_one_class_probability(const svm_problem *prob, const svm_model *model, double *prob_density_marks) {
  ...
  for (int i = 0; i < prob->l; i++) pred_results[i] = svm_predict_values(model, prob->x[i], &dec_values[i]);
  ...
  if (neg_counter < nr_marks / 2 || pos_counter < nr_marks / 2) {
    ...
    int mid           = nr_marks / 2;
    for (int i = 0; i < mid; i++) tmp_marks[i] = dec_values[i * neg_counter / mid];
    tmp_marks[mid] = 0;
    for (int i = mid + 1; i < nr_marks + 1; i++)
      tmp_marks[i] = dec_values[neg_counter - 1 + (i - mid) * pos_counter / mid];

    for (int i = 0; i < nr_marks; i++) prob_density_marks[i] = (tmp_marks[i] + tmp_marks[i + 1]) / 2;
    free(tmp_marks);
  }
  free(dec_values);
  free(pred_results);
  ...
  if (param->svm_type == ONE_CLASS || param->svm_type == EPSILON_SVR || param->svm_type == NU_SVR) {
    ...
    for (i = 0; i < nr_class; i++)
      for (int j = i + 1; j < nr_class; j++) {
        ...
        for (k = 0; k < cj; k++)
          if (!nonzero[sj + k] && fabs(f[p].alpha[ci + k]) > 0) nonzero[sj + k] = true;
        free(sub_prob.x);
        free(sub_prob.y);
        ++p;
      }
...
void svm_cross_validation(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *target) {
  ...
  if (nr_fold > l) {
    fprintf(stderr,
            "WARNING: # folds (%d) > # data (%d). Will use # folds = # data instead (i.e., leave-one-out cross "
            "validation)\n",
            nr_fold, l);
    nr_fold = l;
  }
...
double svm_get_svr_probability(const svm_model *model) {
  if ((model->param.svm_type == EPSILON_SVR || model->param.svm_type == NU_SVR) && model->probA != NULL)
...
double svm_predict_probability(const svm_model *model, const svm_node *x, double *prob_estimates) {
  if ((model->param.svm_type == C_SVC || model->param.svm_type == NU_SVC) && model->probA != NULL &&
      model->probB != NULL) {
    int i;
    int nr_class       = model->nr_class;
    ...
    for (i = 0; i < nr_class; i++)
      for (int j = i + 1; j < nr_class; j++) {
        pairwise_prob[i][j] =
            min(max(sigmoid_predict(dec_values[k], model->probA[k], model->probB[k]), min_prob), 1 - min_prob);
...
int svm_save_model(const char *model_file_name, const svm_model *model) {
  ...
  if (model->prob_density_marks) {
    ...
    int nr_marks = 10;
    for (int i = 0; i < nr_marks; i++) fprintf(fp, " %.17g", model->prob_density_marks[i]);
    fprintf(fp, "\n");
  }
...
bool read_model_header(FILE *fp, svm_model *model) {
  ...
  while (1) {
    ...
    if (strcmp(cmd, "svm_type") == 0) {
      ...
      for (int i = 0; i < n; i++) FSCANF(fp, "%lf", &model->prob_density_marks[i]);
...
void svm_free_model_content(svm_model *model_ptr) {
  if (model_ptr->free_sv && model_ptr->l > 0 && model_ptr->SV != NULL) free((void *)(model_ptr->SV[0]));
...
const char *svm_check_parameter(const svm_problem *prob, const svm_parameter *param) {
  ...
  if (svm_type != C_SVC && svm_type != NU_SVC && svm_type != ONE_CLASS && svm_type != EPSILON_SVR && svm_type != NU_SVR)
    return "unknown svm type";

  // kernel_type, degree

  int kernel_type = param->kernel_type;
  if (kernel_type != LINEAR && kernel_type != POLY && kernel_type != RBF && kernel_type != SIGMOID &&
...
int svm_check_probability_model(const svm_model *model) {
  return ((model->param.svm_type == C_SVC || model->param.svm_type == NU_SVC) && model->probA != NULL &&
          model->probB != NULL) ||
