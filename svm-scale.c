...
#define SKIP_ELEMENT       \
  ...
  while (readline(fp) != NULL) {
    ...
    if (sscanf(p, "%lf", &target) != 1) return clean_up(fp_restore, fp, "ERROR: failed to read labels\n");
      ...
      while (fscanf(fp_restore, "%d %lf %lf\n", &idx, &fmin, &fmax) == 3) {
        for (i = next_index; i < idx; i++)
          if (feature_min[i] != feature_max[i]) {
            fprintf(stderr,
                    "WARNING: feature index %d appeared in file %s was not seen in the scaling factor file %s. The "
                    "feature is scaled to 0.\n",
  ...
  if (save_filename) {
    ...
    for (i = 1; i <= max_index; i++) {
      if (feature_min[i] != feature_max[i]) fprintf(fp_save, "%d %.17g %.17g\n", i, feature_min[i], feature_max[i]);
    }
  ...
  while (readline(fp) != NULL) {
    ...
    if (sscanf(p, "%lf", &target) != 1) return clean_up(NULL, fp, "ERROR: failed to read labels\n");
