...
void predict(FILE *input, FILE *output) {
  ...
  while (readline(input) != NULL) {
    ...
    while (1) {
      ...
      if (endptr == val || errno != 0 || (*endptr != '\0' && !isspace(*endptr))) exit_input_error(total + 1);
...
void exit_with_help() {
  printf(
      ...
      "options:\n"
      "-b probability_estimates: whether to predict probability estimates, 0 or 1 (default 0); for one-class SVM only "
      "0 is supported\n"
